...
const InventoryController = {
    ...
    async createInventory(req, res) {
        const { sku, name, qty, uom, wh, location, imageUrl, unitPrice, exp } = req.body;
        try {
            const newInventory = new inventory({ sku, name, qty, uom, wh, location, imageUrl,unitPrice, exp });
    ...
    async updateInventory(req, res) {
        const { sku } = req.params;
        const { name, qty, uom, wh, location, imageUrl,unitPrice, exp } = req.body;
        try {
            const updatedInventory = await inventory.findOneAndUpdate(
                { sku },
                { name, qty, uom, wh, location, imageUrl,unitPrice, exp },
                { new: true }
            );
            if (!updatedInventory) {
                return res.status(404).json({ message: "Inventory item not found" });
